-- ============================================================================
-- SQL MIGRATION: THIẾT LẬP MỐC XUẤT PHÁT MỚI / RESET ĐIỂM XẾP HẠNG (CUTOFF BASELINE V1)
-- 1. TẠO BẢNG PUBLIC.STUDENT_SCORE_BASELINES
-- 2. TẠO CÁC RPC PREVIEW, APPLY VÀ REVOKE (UNDO) SECURITY DEFINER
-- 3. TÍCH HỢP BỘ LỌC BASELINE VÀO CÁC RPC LEADERBOARD (GAME & ACADEMIC)
...
-- ============================================================================
-- 2. RPC PREVIEW_SCORE_BASELINE_RESET (XEM TRƯỚC TÁC ĐỘNG - READ-ONLY)
-- ============================================================================
...
BEGIN
  ...
  IF v_caller_role IS NULL OR v_caller_role NOT IN ('admin', 'teacher') THEN
    RETURN jsonb_build_object('success', false, 'message', 'Bạn không có quyền thực hiện thao tác này.');
  END IF;
  ...
  IF v_caller_role <> 'admin' AND v_target_class.teacher_id IS DISTINCT FROM v_caller_id THEN
    RETURN jsonb_build_object('success', false, 'message', 'Bạn chỉ có quyền quản lý lớp do mình phụ trách.');
  END IF;

  -- 3. Chuẩn hóa & kiểm tra Scope
  v_scope_clean := LOWER(TRIM(p_scope));
  IF v_scope_clean NOT IN ('game', 'academic', 'both') THEN
    RETURN jsonb_build_object('success', false, 'message', 'Phạm vi reset không hợp lệ (chỉ chấp nhận: game, academic, both).');
  END IF;
  ...
  IF v_from IS NULL THEN
    RETURN jsonb_build_object('success', false, 'message', 'Thời điểm bắt đầu (effective_from) không được để trống.');
  END IF;

  IF v_until IS NOT NULL AND v_until <= v_from THEN
    RETURN jsonb_build_object('success', false, 'message', 'Thời điểm kết thúc (effective_until) phải sau thời điểm bắt đầu.');
  END IF;

  -- 5. Lọc danh sách học sinh hợp lệ thuộc lớp
  ...
  ELSE
    ...
    FROM public.class_members cm
    JOIN public.profiles p ON p.id = cm.student_id
    WHERE cm.class_id = p_class_id AND cm.student_id = ANY(p_student_ids) AND p.role = 'student';
  END IF;

  IF v_valid_student_ids IS NULL OR array_length(v_valid_student_ids, 1) = 0 THEN
    RETURN jsonb_build_object('success', false, 'message', 'Không tìm thấy học sinh hợp lệ nào thuộc lớp này.');
  END IF;
...
-- ============================================================================
-- 3. RPC ADMIN_TEACHER_SET_SCORE_BASELINE (ÁP DỤNG MỐC XUẤT PHÁT MỚI)
-- ============================================================================
...
BEGIN
  ...
  IF v_reason_clean IS NULL OR length(v_reason_clean) = 0 THEN
    RETURN jsonb_build_object('success', false, 'message', 'Vui lòng cung cấp lý do thiết lập mốc xuất phát mới.');
  END IF;
  ...
  -- 6. Lọc danh sách học sinh hợp lệ thuộc lớp
  IF p_student_ids IS NULL OR array_length(p_student_ids, 1) = 0 THEN
    SELECT ARRAY_AGG(cm.student_id)
    INTO v_valid_student_ids
    FROM public.class_members cm
...
BEGIN
  ...
  IF v_target_baseline.id IS NULL THEN
    RETURN jsonb_build_object('success', false, 'message', 'Bản ghi mốc xuất phát không tồn tại.');
  END IF;

  IF v_target_baseline.revoked_at IS NOT NULL THEN
    RETURN jsonb_build_object('success', false, 'message', 'Mốc xuất phát này đã được thu hồi trước đó.');
  END IF;

  -- 3. Kiểm tra phân quyền lớp
  SELECT * INTO v_target_class FROM public.classes WHERE id = v_target_baseline.class_id;
  IF v_target_class.id IS NULL THEN
    RETURN jsonb_build_object('success', false, 'message', 'Lớp học liên quan không tồn tại.');
  END IF;

  IF v_caller_role <> 'admin' AND v_target_class.teacher_id IS DISTINCT FROM v_caller_id THEN
    RETURN jsonb_build_object('success', false, 'message', 'Bạn không có quyền thu hồi mốc xuất phát của lớp này.');
  END IF;
...
BEGIN
  ...
  SELECT role INTO v_caller_role FROM public.profiles WHERE id = v_caller_id;
  IF v_caller_role IS NULL OR v_caller_role NOT IN ('admin', 'teacher') THEN
    RETURN jsonb_build_object('success', false, 'message', 'Bạn không có quyền truy cập.');
  END IF;
  ...
  IF v_caller_role <> 'admin' AND v_target_class.teacher_id IS DISTINCT FROM v_caller_id THEN
    RETURN jsonb_build_object('success', false, 'message', 'Bạn chỉ có quyền xem lớp do mình phụ trách.');
  END IF;
...
-- ============================================================================
-- 6. CẬP NHẬT CÁC RPC BẢNG XẾP HẠNG TRÒ CHƠI (TÍCH HỢP BASELINE FILTER)
-- ============================================================================
...
BEGIN
  ...
  valid_game_students AS (
    SELECT 
      ...
      p.avatar_url,
      CASE 
        WHEN v_requested_class_uuid IS NOT NULL AND EXISTS (
          SELECT 1 FROM public.student_score_baselines b
          WHERE b.student_id = p.id AND b.class_id = v_requested_class_uuid AND b.revoked_at IS NULL AND b.scope IN ('game', 'both')
        ) THEN (
          ...
          WHERE sp.student_id = p.id
            AND NOT EXISTS (
              ...
              WHERE b.student_id = p.id
                AND b.class_id = v_requested_class_uuid
...
-- ============================================================================
-- 7. CẬP NHẬT CÁC RPC BẢNG XẾP HẠNG HỌC THUẬT (TÍCH HỢP BASELINE FILTER)
-- ============================================================================
...
BEGIN
  ...
  -- NẾU DRAFT / ACTIVE -> TÍNH ĐỘNG CÓ ÁP DỤNG BASELINE
  WITH valid_assignments AS (
    SELECT 
      a.exercise_id,
      e.title,
