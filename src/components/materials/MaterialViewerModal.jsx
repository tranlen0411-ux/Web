...
export const MaterialViewerModal = ({ isOpen, onClose, material }) => {
  ...
  useEffect(() => {
    ...
    const hasOuterResizeExperimentRunRef = { current: false };

    const executeOuterPlayerNativeResizeExperiment = () => {
      if (hasOuterResizeExperimentRunRef.current) return;
      hasOuterResizeExperimentRunRef.current = true;

      const outerIframe = scormIframeRef.current;
      if (!outerIframe) {
        console.warn('[SCORM OUTER] outerIframe not found');
        return;
      }

      const origW = outerIframe.clientWidth;
      const origH = outerIframe.clientHeight;
      const origStyleW = outerIframe.style.width;

      let playerWin = null;
      try {
        playerWin = outerIframe.contentWindow;
      } catch {
        // ignore
      }

      console.log(`[SCORM OUTER] initial outerIframe=${origW}x${origH}`);
      console.log(`[SCORM OUTER] playerWindow inner=${playerWin?.innerWidth || 'cross-origin'}x${playerWin?.innerHeight || 'cross-origin'}`);
      ...
      const cleanupAndRestore = () => {
        ...
        try {
          if (playerWin) {
            playerWin.removeEventListener('resize', handleFirstOuterNativeResize);
            playerWin.removeEventListener('resize', handleRestoreOuterNativeResize);
          }
        } catch {
          // ignore
        }
        outerIframe.style.width = origStyleW || '';
      };

      const handleFirstOuterNativeResize = () => {
        try {
          playerWin?.removeEventListener('resize', handleFirstOuterNativeResize);
        } catch {
          // ignore
        ...
        console.log(`[SCORM OUTER] NATIVE PLAYER RESIZE RECEIVED`);
        console.log(`[SCORM OUTER] outerIframe=${outerIframe.clientWidth}x${outerIframe.clientHeight}`);
        console.log(`[SCORM OUTER] playerWindow inner=${playerWin?.innerWidth || 'cross-origin'}x${playerWin?.innerHeight || 'cross-origin'}`);
        ...
        requestAnimationFrame(() => {
          requestAnimationFrame(() => {
            try {
              let scoSize = 'NA';
              let slidesBgRect = 'NA';
              let framesLayerRect = 'NA';
              let svgCount = 0;
              let contentVisible = 'NA';

              try {
                const playerDoc = playerWin?.document;
                const scoIframe = playerDoc?.getElementById('scorm-content-frame');
                const scoWin = scoIframe?.contentWindow;
                const scoDoc = scoIframe?.contentDocument || scoWin?.document;

                if (scoWin) {
                  scoSize = `${scoWin.innerWidth}x${scoWin.innerHeight}`;
                }
                if (scoDoc) {
                  const slidesBg = scoDoc.querySelector('#slidesBackground, [id*="slidesBackground"]');
                  const framesLayer = scoDoc.querySelector('.framesLayerContent, [class*="framesLayer"]');
                  const svgs = Array.from(scoDoc.querySelectorAll('svg'));
                  svgCount = svgs.length;

                  if (slidesBg) {
                    const r = slidesBg.getBoundingClientRect();
                    slidesBgRect = `[${Math.round(r.left)},${Math.round(r.top)},${Math.round(r.width)}x${Math.round(r.height)}]`;
                    contentVisible = r.width > 10 ? 'YES' : 'NO';
                  }
                  if (framesLayer) {
                    const r = framesLayer.getBoundingClientRect();
                    framesLayerRect = `[${Math.round(r.left)},${Math.round(r.top)},${Math.round(r.width)}x${Math.round(r.height)}]`;
                  }
                }
              } catch {
                console.log('[SCORM OUTER] note: inspecting via player internal diagnostic logs');
              }

              console.log(`[SCORM OUTER] AFTER_PLAYER_NATIVE_RESIZE`);
              console.log(`  sco=${scoSize}`);
              ...
              // Phase 2: Restore kích thước Outer Iframe
              try {
                if (playerWin) {
                  playerWin.addEventListener('resize', handleRestoreOuterNativeResize);
                }
              } catch {
                // ignore
              }
              outerIframe.style.width = origStyleW || '';
              console.log('[SCORM OUTER] restore outerIframe requested, waiting for restore native event...');
            } catch (err) {
              console.warn('[SCORM OUTER] Error after player native resize:', err.message);
              cleanupAndRestore();
            }
          });
        });
      ...
      const handleRestoreOuterNativeResize = () => {
        try {
          playerWin?.removeEventListener('resize', handleRestoreOuterNativeResize);
        } catch {
          // ignore
        ...
        console.log(`[SCORM OUTER] RESTORE PLAYER NATIVE RESIZE RECEIVED`);

        requestAnimationFrame(() => {
          requestAnimationFrame(() => {
            let finalContentVisible = 'NA';
            try {
              const playerDoc = playerWin?.document;
              const scoIframe = playerDoc?.getElementById('scorm-content-frame');
              const scoDoc = scoIframe?.contentDocument || scoIframe?.contentWindow?.document;
              const slidesBg = scoDoc?.querySelector('#slidesBackground, [id*="slidesBackground"]');
              if (slidesBg) {
                finalContentVisible = slidesBg.getBoundingClientRect().width > 10 ? 'YES' : 'NO';
              }
            } catch {
              // ignore
            }

            console.log(`[SCORM OUTER] final outerIframe=${outerIframe.clientWidth}x${outerIframe.clientHeight}`);
            console.log(`[SCORM OUTER] final playerWindow=${playerWin?.innerWidth || 'cross-origin'}x${playerWin?.innerHeight || 'cross-origin'}`);
            console.log(`[SCORM OUTER] final contentVisible=${finalContentVisible}`);
          });
        });
      };
      ...
      try {
        if (playerWin) {
          playerWin.addEventListener('resize', handleFirstOuterNativeResize);
          console.log('[SCORM OUTER] listener armed');
        }

        // Tạo 1px delta trên Outer Player Iframe
        const targetPx = (origW > 10 ? origW - 1 : origW + 1) + 'px';
        outerIframe.style.width = targetPx;

        // Fallback safety timeout 2.5s
        timeoutId = setTimeout(() => {
          if (!isCleanedUp) {
            console.warn('[SCORM OUTER] EVENT_NOT_RECEIVED (timeout 2.5s)');
            cleanupAndRestore();
          }
        }, 2500);
      } catch (armErr) {
        console.warn('[SCORM OUTER] Fatal error:', armErr.message);
        cleanupAndRestore();
      }
    };
    ...
    const handleMessage = async (event) => {
      ...
      if (
        ...
        if (msgType === 'SCORM_LOADED') {
          setTimeout(() => {
            executeOuterPlayerNativeResizeExperiment();
          }, 600);
        }
