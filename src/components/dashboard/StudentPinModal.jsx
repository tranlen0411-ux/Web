...
export const StudentPinModal = ({ isOpen, onClose, student, onSuccess }) => {
  ...
  const [createdPinForDisplay, setCreatedPinForDisplay] = useState(null);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Xóa form inputs và lưu PIN vừa tạo vào state hiển thị tạm thời 1 lần
      setNewPin('');
      setConfirmPin('');
      setCreatedPinForDisplay(cleanNewPin);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fadeIn">
      <div className="relative w-full max-w-md bg-white rounded-3xl border-4 border-amber-300 p-6 shadow-2xl">
        ...
        {checking ? (
          ...
          <>
            {/* Nếu vừa tạo PIN thành công -> Hiển thị PIN 1 lần */}
            {createdPinForDisplay ? (
              <div className="space-y-4">
                <div className="p-4 bg-emerald-50 border-2 border-emerald-300 text-emerald-900 rounded-2xl text-center">
                  <CheckCircle2 className="w-8 h-8 text-emerald-600 mx-auto mb-1.5" />
                  <h4 className="font-black text-base text-emerald-950">
                    {hasPin ? 'Reset Mã PIN Thành Công!' : 'Thiết Lập Mã PIN Thành Công!'}
                  </h4>
                  <p className="text-xs font-bold text-emerald-700 mt-0.5">
                    Mã PIN mới của học sinh <span className="font-black text-emerald-950">{student.full_name}</span>:
                  </p>

                  <div className="my-3 py-3 px-4 bg-white border-2 border-dashed border-emerald-400 rounded-2xl inline-block shadow-inner">
                    <span className="font-mono text-3xl font-black text-emerald-800 tracking-[0.25em]">
                      {createdPinForDisplay}
                    </span>
                  </div>

                  <div className="p-3 bg-amber-50 border border-amber-300 rounded-xl text-[11px] font-bold text-amber-900 text-left flex items-start gap-2">
                    <span className="text-sm">⚠️</span>
                    <span>
                      Mã PIN chỉ hiển thị một lần. Hãy ghi lại hoặc gửi cho học sinh trước khi đóng.
                    </span>
                  </div>
                </div>
                ...
                <button
                  type="button"
                  onClick={handleClose}
                  className="w-full py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-black text-xs rounded-xl border-b-4 border-emerald-800 shadow-md active:translate-y-0.5 transition-all"
                >
                  ✓ ĐÃ LƯU MÃ PIN — HOÀN TẤT & ĐÓNG
                </button>
              </div>
            ) : (
              <>
