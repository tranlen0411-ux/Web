...
-- CHỈ CHO PHÉP GIAO/THU HỒI BÀI QUA RPC SECURITY DEFINER (KHÓA INSERT/UPDATE DIRECT)
-- ============================================================================
...
-- 3. CẤP QUYỀN TRUY CẬP AN TOÀN - REVOKE TOÀN BỘ QUYỀN TỪ ANON, BẢNG CHỈ CHO AUTHENTICATED SELECT
...
GRANT SELECT ON public.academic_exercise_assignments TO authenticated;
...
-- SIẾT BẢO MẬT RPC GIAO BÀI - CHỈ CHO PHÉP TÀI KHOẢN ĐÃ XÁC THỰC (AUTHENTICATED) GỌI HÀM
...
-- 9. RPC DEFINITION: THU HỒI BÀI TẬP KHỎI DANH SÁCH LỚP HỌC (UNASSIGN ATOMIC)
CREATE OR REPLACE FUNCTION public.unassign_exercise_from_classes(
  p_exercise_id UUID,
  p_class_ids UUID[]
)
RETURNS JSONB
...
DECLARE
  ...
  v_class_id UUID;
  v_class_record RECORD;
  v_unassigned_names TEXT[] := ARRAY[]::TEXT[];
  v_failed_names TEXT[] := ARRAY[]::TEXT[];
  v_unique_class_ids UUID[];
BEGIN
  v_caller_id := auth.uid();
  ...
  SELECT role INTO v_caller_role FROM public.profiles WHERE id = v_caller_id;
  IF v_caller_role IS NULL OR v_caller_role NOT IN ('admin', 'teacher') THEN
    RETURN jsonb_build_object('success', false, 'message', 'Bạn không có quyền thu hồi bài tập.');
  END IF;
  ...
  IF p_class_ids IS NULL OR array_length(p_class_ids, 1) = 0 THEN
    RETURN jsonb_build_object('success', false, 'message', 'Vui lòng chọn ít nhất 1 Lớp học để thu hồi.');
  END IF;
  ...
  LOOP
    ...
    DELETE FROM public.academic_exercise_assignments
    WHERE exercise_id = p_exercise_id AND class_id = v_class_id;

    v_unassigned_names := array_append(v_unassigned_names, v_class_record.name);
  END LOOP;

  RETURN jsonb_build_object(
    'success', true,
    'unassigned_classes', to_jsonb(v_unassigned_names),
    'failed_classes', to_jsonb(v_failed_names),
    'message', 'Đã thu hồi bài tập khỏi các lớp được chọn thành công.'
  );
END;
$$;

-- SIẾT BẢO MẬT RPC THU HỒI - CHỈ CHO PHÉP TÀI KHOẢN ĐÃ XÁC THỰC (AUTHENTICATED) GỌI HÀM
REVOKE ALL ON FUNCTION public.unassign_exercise_from_classes(UUID, UUID[]) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.unassign_exercise_from_classes(UUID, UUID[]) TO authenticated;

COMMIT;

-- 10. NẠP LẠI SCHEMA CACHE TRÊN SUPABASE POSTGREST ĐỂ CẬP NHẬT NGAY LẬP TỨC
