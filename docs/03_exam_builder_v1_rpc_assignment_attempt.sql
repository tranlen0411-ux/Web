...
-- ------------------------------------------------------------
-- 1. rpc_exam_create_assignment (Teacher Exam Assignment)
-- ------------------------------------------------------------
-- Binds a published exam_version to a class_id from CORE.
-- Target exam_version MUST have status = 'published' (superseded/draft rejected).
-- Requires stable p_assignment_id generated by BFF for idempotency.
-- Only exam author or Admin may assign the exam.
...
BEGIN
    ...
    -- 2. Lock & fetch target exam_version
    ...
    -- 3. Lock & fetch parent exam container
    ...
    -- 4. Authorization check: Only exam author or Admin can assign
    ...
    -- 5. Due date override validation: assignment due_date must not exceed version due_date
    ...
    -- 6. Check unique(exam_version_id, class_id) under a DIFFERENT assignment_id
    ...
    -- 7. Insert assignment using ON CONFLICT (id) DO NOTHING for PK idempotency
    ...
    IF FOUND THEN
        v_inserted := TRUE;
    ELSE
        -- Conflict on assignment id: lock & verify exact replay
        SELECT * INTO v_existing_assignment
        FROM public.exam_assignments
        WHERE id = p_assignment_id
        FOR UPDATE;

        IF v_existing_assignment.exam_version_id <> p_exam_version_id OR
           v_existing_assignment.class_id <> p_class_id OR
           v_existing_assignment.assigned_by <> p_caller_id OR
           v_existing_assignment.due_date IS DISTINCT FROM p_due_date OR
