-- docs/QUESTION_BANK_TEACHER_PUBLISH_SHARING.sql
-- QUESTION BANK — TEACHER PUBLISH SHARING HOTFIX V1
-- Target Database: szptvqkoiphrhlionfoh (Supabase NEW)
-- Allows teachers to set visibility to 'public_template' on their own questions.
...
BEGIN
  ...
  -- 4. Kiểm tra quyền thay đổi visibility
  -- Hotfix V1: Teacher ĐƯỢC PHÉP đặt 'public_template' trên câu hỏi của chính mình.
  -- school_shared: Duy trì quy tắc fail-closed nếu không có school_id hợp lệ gắn với item.
  ...
  -- 5. Hardening kiểm soát chuyển đổi trạng thái (status transitions)
